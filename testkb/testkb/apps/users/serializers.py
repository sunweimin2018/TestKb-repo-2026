...
class CreateUserSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        print("user in CreateUserSerializer first:", validated_data)
        user.set_password(password)  # 把密码加密后再赋值给user的password属性
        user.save()  # 存储到数据库

        print("user in CreateUserSerializer:",user)
        refresh=RefreshToken.for_user(user)
        print("refresh in usercreat:",refresh)
        token=str(refresh.access_token)
        # token = SlidingToken.for_user(user)
        print("token in CreateUserSerializer",token)
        user.token = token
        print("this is user:")
        print(user.password)
        return user
